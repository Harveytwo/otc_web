$('.trade_rank_item_right span').on('click', function() {
    $(this).parents('.trade_rank_item').toggleClass('unfold').siblings().removeClass('unfold');
    // $('.trade_rank_lists ul').toggleClass('unfold_li');
})
$('.change_lan,.use_login').on('mouseover', function() {
...
(function(w) {
    ...
    var cityArr = [];
    cityArr[0] = ['东城区', '西城区', '崇文区', '宣武区', '朝阳区', '丰台区', '石景山区', '海淀区', '门头沟区', '房山区', '通州区', '顺义区', '昌平区', '大兴 区', '怀柔区', '平谷区', '密云县', '延庆县'];
    cityArr[1] = ['和平区', '河东区', '河西区', '南开区', '河北区', '红桥区', '塘沽区', '汉沽区', '大港区', '东丽区', '西青区', '津南区', '北辰区', '武清区', '宝坻区', '宁河县', '静海县', '蓟县'];
    cityArr[2] = ['黄浦区', '卢湾区', '徐汇区', '长宁区', '静安区', '普陀区', '闸北区', '虹口区', '杨浦区', '闵行区', '宝山区', '嘉定区', '浦东新区', '金山区', '松江区', '青浦区', '南汇区', '奉贤区', '崇明县'];
    cityArr[3] = ['万州区', '涪陵区', '渝中区', '大渡口区', '江北区', '沙坪坝区', '九龙坡区', '南岸区', '北碚区', '万盛区', '双桥区', '渝北区', '巴南区', '黔江区', '长寿区', '江津区', '合川区', '永川区', '南川区', '綦江县', '潼南县', '铜梁县', '大足县', '荣昌县', '璧山县', '梁平县', '城口县', '丰都县', '垫江县', '武隆县', '忠县', '开县', '云阳县', '奉节县', '巫山县', '巫溪县', '石柱土家族自治县', '秀山土家族苗族自治县', '酉阳土家族苗族自治县', '彭水苗族土家族自治县'];
    ...
    cityArr[32] = ['中西区', '湾仔区', '东区', '南区', '油尖旺区', '深水埗区', '九龙城区', '黄大仙区', '观塘区', '荃湾区', '葵青区', '沙田区', '西贡区', '大埔区', '北区', '元朗区', '屯门区', '离岛区'];
    cityArr[33] = ['澳门'];
    var _li = '';
    for(var i=0; i<provinceArr.length; i++) {
        _li = _li + '<li>' + provinceArr[i] + '</li>'
    }
    $('.province > .otc_content_filter_word_wp ul').append(_li);
    w.cityArr = cityArr;
})(window);

$('.otc_content_filter_word_wp').on('click', 'li',function() {
    var _text = $(this).text();
    var that = $(this);
    that.parents('.otc_content_filter_word_item').find('span').text(_text);
    console.log(_text)

    if(that.parents('.otc_content_filter_word_item').hasClass('province')) {
        var _index = that.index()-1;
        $('.city>span').text('城市');
        if(_index >= 0) {
            var _cityArr = cityArr[_index];
            var _li = '';
            for(var i=0; i<_cityArr.length; i++) {
                _li = _li + '<li>' + _cityArr[i] + '</li>'
            }
            $('.city > .otc_content_filter_word_wp ul')[0].innerHTML = '<li>所有</li>';
            $('.city > .otc_content_filter_word_wp ul').append(_li);
        } else {
            $('.city > .otc_content_filter_word_wp ul')[0].innerHTML = '<li>所有</li>';
        }
    }
